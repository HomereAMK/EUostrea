...
pca_summary <- c(sum(c^2), e_values[1:pc], unlist(e_vectors[1:pc], use.names=FALSE)) %>%
	matrix(nrow = 1) %>%
	as.data.frame()
write_tsv(pca_summary, paste0(out_dir, "pca_summary14feb23_", snp, "snp_", lg, "_", pc, "pc.tsv"), append=T, col_names=F)
...
snp_position <- c(b[[1,1]], b[[dim(b)[1],1]]) %>%
	matrix(nrow = 1) %>%
	as.data.frame()
write_tsv(snp_position, paste0(out_dir, "snp_position14feb23_", snp, "snp_", lg, "_", pc, "pc.tsv"), append=T, col_names=F)